...
def time_CPU(opt_model, model, device, input_batch, reps, iters):
    file_name = PATH + MODEL_NAME + "/" + "results-" + MODEL_NAME + "-optimized-" + str(device) + ".txt"
    f = open(file_name, "w")
    results = ""
    ...
    file_name = PATH + MODEL_NAME + "/" + "results-" + MODEL_NAME + "-unoptimized-" + str(device) + ".txt"
    f = open(file_name, "w")
    results = ""
...
def profiler(opt_model, model, device, input_batch, useCuda):
    file_name = PATH + MODEL_NAME + "/" + "profile-" + MODEL_NAME + "-unoptimized-" + str(device) + ".txt"
    f = open(file_name, "w")
    ...
    file_name = PATH + MODEL_NAME + "/" + "profile-" + MODEL_NAME + "-optimized-" + str(device) + ".txt"
    f = open(file_name, "w")
    with torch.autograd.profiler.profile(use_cuda=useCuda) as prof:
...
def time_cuda(opt_model, model, device, input_batch, reps, iters):
    print("Unoptimized Model:")
    model_results = measure_cuda(model, device, input_batch, reps, iters)
    print("\nOptimized Model")
    opt_model_results = measure_cuda(opt_model, device, input_batch, reps, iters)
...
def get_test_image():
    url, file_name = ("https://github.com/pytorch/hub/raw/master/images/dog.jpg", "dog.jpg")
...
def main():
    ...
    if (device == "cpu"):
        ...
        time_CPU(opt_model=opt_model, model=model, device=device, input_batch=input_batch, reps=NUM_REPS, iters=NUM_ITERS)
    else:
        time_cuda(opt_model=opt_model, model=model, device=device, input_batch=input_batch, reps=NUM_REPS, iters=NUM_ITERS)

    # Profile execution
    profiler(opt_model=opt_model, model=model, device=device, input_batch=input_batch, useCuda=use_cuda)
