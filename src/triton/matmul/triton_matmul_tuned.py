...
def test():
    ...
    triton_output = matmul_tuned(a, b)
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_vals=['cublas', 'triton-tuned', 'triton-untuned'],
        # Label name for the lines
        line_names=["cuBLAS", "Triton AutoTuning", "Triton No Tuning"],
        # Line styles
...
def benchmark(M, N, K, provider):
    ...
    if provider == 'triton-tuned':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: matmul_tuned(a, b), quantiles=quantiles)
